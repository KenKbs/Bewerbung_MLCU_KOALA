...
# Goal of this Repo
* We want to build a quick prototype, so I can apply on the Job description in reports/Ausschreibung-HiWi(ne)-KOALA.docx.pdf 
* It should be kept MINIMAL Goal is to get this working in around 4hours
* remember, this is just a small voluntary work from my side to attach to my application to improve my chances of getting this position. 
* it's not a final project, it's just to demonstrate my skills and that I understood what this project is about

# Concrete Requirements:
* Load sample polls (no real pools, just some sample.csv data)
* Compute weighted polling average
* Model uncertainty- not perfect, but plausible, some multivariate nNormal with correlation Matrix. It's important to indicate the correlation, so that we can proof that I understood the problem
* Monte carlo simulation e.g. draw 10000 samples
* Apply the 5 % electoral threshold 
* Display coalation probabilites 
* Bar chart + a brief explanation
* Everything wrapped in a small Streamlit app for easy display